...
//  SDCard_scanFolder("/", maxItems, maxPathLength, scanOutput);
  ...
  while (1)
  {
	...
	result = SDCard_updateFile("FILE1.TXT", buffer);
	...
	result = SDCard_updateFile("FILE2.TXT", buffer);
	...
	// Only works if the debug line connected to STLink
